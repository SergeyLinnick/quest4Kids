...
export async function refreshAccessToken(token: JWT): Promise<JWT> {
  console.log("Old refresh_token ======>", token.refreshToken);
  ...
  try {
    ...
    return {
      ...
      //   : Date.now() + 3600 * 1000,
      // Force token to expire in 2 minutes for testing
    ...
    ZitadelProvider({
      ...
      authorization: {
        params: {
          ...
          // Force user consent to ensure offline_access (refresh token) is granted
          prompt: "consent",
        },
      },
    }),
